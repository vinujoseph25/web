...
babbleApp.controller 'TableWidget', ($scope, $location, $window, dashboardService, dataService, logService) ->
    ...
    if $scope.widget.name?
        $window.Babble.currentPage.widgets[$scope.widget.name].goToPage = (pageNumber) ->
            $scope.$evalAsync ->
