...
#
babbleApp.controller 'AnnotationChartWidget', ($scope, configService, babbleDataService, dashboardService, dataService, logService) ->
    ...
    $scope.updateChart = (data) ->
        ...
        # Merge Annotation Data from BabbleData (if available)
    ...
    $scope.saveAnnotation = ->
        ...
        if $scope.selectedPoint.existingAnnotation?
            ...
            babbleDataService.upsert(key, matchingKeys, $scope.selectedPoint.existingAnnotation).then ->
            ...
            babbleDataService.append(key, newAnnotation).then ->
    ...
    $scope.deleteAnnotation = ->
        ...
        babbleDataService.remove(key, matchingKeys).then ->
            babbleDataService.upsert(key, matchingKeys, $scope.selectedPoint.existingAnnotation).then ->
    ...
    if $scope.dataSource?
        ...
        if $scope.widget.annotationEditing == true and !_.isEmpty($scope.widget.annotationKey)
            # Load annotations from BabbleData
            babbleDataService.getBucketData($scope.widget.annotationKey).then (annotationData) ->
