...
babbleDirectives.directive 'headerParameter', ($window, dashboardService, dataService, logService) ->
    {
        restrict: 'CA'
        scope: 
            dashboard: '='
        ...
        controller: ($scope) ->
            ...
            originalValue = $window.Babble.parameters[$scope.parameterDefinition.name]
            ...
            $scope.updateParameter = ->
                if not _.isEqual $scope.parameter.value, originalValue
                    $window.Babble.parameters[$scope.parameterDefinition.name] = $scope.parameter.value
