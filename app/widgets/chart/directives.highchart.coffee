...
Highcharts.wrap Highcharts.Axis.prototype, 'getPlotLinePath', (proceed) ->
  path = proceed.apply(this, Array::slice.call(arguments, 1))
  if path
    path.flat = false
  path

# Inspired by: https://github.com/rootux/angular-highcharts-directive
babbleDirectives.directive 'highchart', (configService) ->
    {
        restrict: 'CA',
        replace: false,
        scope:
        ...
        link: (scope, element, attrs) ->
            ...
            scope.$watch('chart', (chart) ->
                ...
                # Create or Update
                if highchartsObj? && _.isEqual(_.omit(scope.currentChart, 'series','yAxis'), _.omit(chart, 'series','yAxis'))
