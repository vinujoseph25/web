...
babbleServices.factory 'userService', ($http, $localForage, $q, $rootScope, $window, configService, cryptoService, logService) ->
    ...
    exports = {
        ...
        setLoggedOut: ->
            ...
            if $window.Babble?
                $window.Babble.currentUser = null
                $window.Babble.currentUsername = null
                $window.Babble.currentUserPassword = null
    ...
    $localForage.getItem('username').then (username) ->
        if username?
            ...
            if $window.Babble?
                $window.Babble.currentUsername = username
    ...
    $localForage.getItem('cachedPassword').then (cachedPassword) ->
        if cachedPassword? and configService.authentication.cacheEncryptedPassword
            ...
            if $window.Babble?
                $window.Babble.currentUserPassword = cachedPassword
    ...
    exports.login = (username, password) ->
        ...
        post.success (session) ->
            ...
            if $window.Babble?
                $window.Babble.currentUsername = username
                $window.Babble.currentUser = session.user
            ...
            if (configService.authentication.cacheEncryptedPassword)
                ...
                cryptoService.encrypt(password).then (encrypedPassword) ->
                    ...
                    if $window.Babble?
                        $window.Babble.currentUserPassword = encrypedPassword
