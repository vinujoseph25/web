...
babbleServices.factory 'dashboardService', ($http, $resource, $q, analyticsService, configService, userService) ->
    ...
    service.getDashboards = (query) ->
        ...
        p.catch (error) ->
            alertify.error(error?.data || 'Cannot connect to babble-service (getDashboards)', 2500)
    ...
    service.delete = (name) ->
        ...
        p.catch (error) ->
            switch error.status
                ...
                when 402
                    alertify.error("Widget count exceeded. Please contact administrator")
                else
                    alertify.error(error.data, 2500)
            deferred.reject error
    ...
    service.getRevisions = (dashboardName) ->
        ...
        p.catch (error) ->
            alertify.error(error?.data || 'Cannot connect to babble-service (getRevisions)', 2500)
            deferred.reject error
    ...
    service.getRevisionDiff = (dashboardName, rev1, rev2) ->
        ...
        .catch (error) ->
            alertify.error(error?.data || 'Cannot connect to babble-service (getRevisionDiff)', 2500)
            deferred.reject error
    ...
    service.getRevision = (dashboardName, rev) ->
        ...
        p.catch (error) ->
            alertify.error(error?.data || 'Cannot connect to babble-service (getRevision)', 2500)
            deferred.reject error
    ...
    #
    # Push the dashboard to any specified Babble endpoint.
    #
