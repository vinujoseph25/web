...
#
# Manages Babble.parameters
#
babbleServices.factory 'parameterService', ($localForage, $q, $window, configService, logService) ->
    
    getLocalStorageKey = (dashboard, parameterDefinition) ->
        'param.' + dashboard.name + '.' +  parameterDefinition.name

    isSet = (parameterDefinition) ->
        $window.Babble.parameters[parameterDefinition.name]?

    setValue = (parameterDefinition, value) ->
        $window.Babble.parameters[parameterDefinition.name] = value
        ...
        initializeParameters: (dashboard) ->
            return $q (resolve, reject) ->
                ...
                $q.all(qs).then ->
                    ...
                    # Log all parameters
                    _.each $window.Babble.parameters, (value, key) ->
