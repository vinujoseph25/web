...
#
# Provides access to the BabbleData API
babbleServices.factory 'babbleDataService', ($http, configService, logService) ->
    defaultUrl = configService.restServiceUrl

    checkBucketExists = (key, url = defaultUrl) ->
        logService.debug 'BabbleData: Checking if bucket exists:', key
    ...
    createBucket = (key, data = [], url = defaultUrl) -> 
        logService.debug 'BabbleData: Creating new bucket:', key
        url = url + '/data'
