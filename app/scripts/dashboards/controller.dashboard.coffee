...
# Home controller.
#
babbleApp.controller 'DashboardController', ($scope, $stateParams, $localForage, $location, $timeout, $window, $q, $uibModal, analyticsService, configService, babbleDataService, dashboardOverridesService, dashboardService, dataService, downloadService, loadService, logService, parameterService, userService) ->
    ...
    # Create Babble global
    # Load URL querystring into Babble.parameters
    $window.Babble =
    ...
    $scope.updateUrl = ->
        ...
        loadService.setTitle $scope.dashboardDisplayName + ' | ' + pageName + ' | Babble'
        ...
        Babble.dashboardName = $scope.dashboard.name
        Babble.pageName = pageName
    ...
    $scope.rotate = ->          
        ...
        $window.Babble.parameters.page = pageNumber
    ...
    handleQueryStringChanges = (parameters, oldParameters) ->
        ...
        logService.debug 'Updating Babble.parameters from querystring'

        _.assign($window.Babble.parameters, parameters)

        # Remove deleted parameters
        deletedKeys = _.difference(_.keys(oldParameters), _.keys(parameters))
        _.each deletedKeys, (key) ->
            logService.debug 'Removing parameter from Babble.parameters: ' + key
            delete Babble.parameters[key]
            return
    ...
    handleParameterChanges = (parameters, oldParameters) ->
        return if _.isEqual(parameters, oldParameters)
        logService.debug 'Updating querystring from Babble.parameters'
    ...
    $scope.loadDashboard = (deferred = $q.defer()) ->
        ...
        q = dashboardService.getDashboard($scope.originalDashboardName, $window.Babble.parameters.rev)
        q.then (dashboardWrapper) ->
            ...
            $scope.dashboardWrapper = Babble.dashboard = dashboardWrapper
    ...
    $scope.initialLoad = ->
        ...
        dashboardOverridesService.initializeDashboardOverrides($scope.dashboard).then (dashboardOverrides) ->
            ...
            $window.Babble.dashboardOverrides = $scope.dashboardOverrides = dashboardOverrides
            ...
            $scope.resetDashboardOverrides = ->
                $window.Babble.dashboardOverrides = $scope.dashboardOverrides = dashboardOverridesService.resetAndExpandOverrides $scope.dashboard
            ...
            parameterService.initializeParameters($scope.dashboard).then ->
                ...
                $scope.$watch (-> $window.Babble.parameters), handleParameterChanges, true
                ...
                if $scope.dashboard.pages.length > 0
                    ...
                    if $window.Babble.parameters.page?
                        $scope.goToPage parseInt($window.Babble.parameters.page)
                ...
                if $window.Babble.parameters.autoRotate == "true" or $scope.dashboard.autoRotate == true
    ...
    $scope.reloadInterval = if $window.Babble.parameters.live == 'true' then 1500 else 60000

    # Helper functions
    $window.Babble.goToPage = $scope.goToPage
    $window.Babble.pause = $scope.pause
    $window.Babble.getDeeplink = -> 
        new URI()
